...
namespace VectorTiles.RBush
{
	public partial class RBush<T>
	{
		...
		private static readonly IComparer<ISpatialData> CompareMinX =
			ProjectionComparer<ISpatialData>.Create(d => d.Envelope.Left);
		private static readonly IComparer<ISpatialData> CompareMinY =
			ProjectionComparer<ISpatialData>.Create(d => d.Envelope.Top);
		#endregion

		#region Search
		private List<ImmutableStack<ISpatialData>> DoPathSearch(in SKRect boundingBox)
		{
		...
		private List<T> DoSearch(in SKRect boundingBox)
		{
			if (!Root.Envelope.Intersects(boundingBox))
				return new List<T>();
		...
		#region Insert
		private List<Node> FindCoveringArea(in SKRect area, int depth)
		{
			...
			while (true)
			{
				...
				node = node.children
					.Select(c => new { EnlargedArea = c.Envelope.Extend(_area).Area(), Area = c.Envelope.Area(), Node = c as Node, })
		...
		private static SKRect GetEnclosingEnvelope(IEnumerable<ISpatialData> items)
		{
			var envelope = SKRect.Empty;
